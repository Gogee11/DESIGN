...
class Trainer(object):
    ...
    def train(self, model):
        ...
        self._dump_metrics(best_epoch, test_result)

    def _dump_metrics(self, best_epoch, test_result):
        try:
            tag = configs.get('tag') or ''
            tag_suffix = f"-{tag}" if tag and tag != 'default' else ''
            out_dir = './results/utility'
            os.makedirs(out_dir, exist_ok=True)
            ks = configs['test'].get('k', [5, 10, 20])
            record = {
                'kind': 'utility',
            ...
            out = os.path.join(out_dir, f"utility-{configs['model']['name']}-{configs['data']['name']}-{configs['train']['seed']}{tag_suffix}.json")
            with open(out, 'w', encoding='utf-8') as f:
                json.dump(record, f, ensure_ascii=False, indent=2)
        except Exception as e:
            self.logger.log(f"[warn] failed to dump metrics json: {e}")
        ...
        if configs['train']['save_model']:
            ...
            if not configs['tune']['enable']:
                ...
                save_path = '{}/{}-{}-{}{}.pth'.format(
                    ...
                    configs['data']['name'], configs['train']['seed'], tag_suffix)
